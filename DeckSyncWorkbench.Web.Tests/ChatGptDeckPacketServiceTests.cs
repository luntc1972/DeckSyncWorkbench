...
""",
    ...
    {
        var artifactsRoot = Path.Combine(Path.GetTempPath(), "DeckSyncWorkbenchTests", Guid.NewGuid().ToString("N"));
...
""",
    ...
    {
        var rootPath = contentRootPath ?? Path.Combine(Path.GetTempPath(), "DeckSyncWorkbenchTests", Guid.NewGuid().ToString("N"));
...
""");
    ...
    {
        public string ApplicationName { get; set; } = "DeckSyncWorkbench.Tests";
