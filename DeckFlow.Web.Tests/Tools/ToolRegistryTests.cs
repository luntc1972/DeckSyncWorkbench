...
/// <summary>
/// Verifies the tool registry contract for Phase 66.
/// </summary>
public sealed class ToolRegistryTests
{
    [Fact]
    public void All_ReturnsExpectedToolDefinitions()
    {
        ...
        Assert.Collection(
            ...
            tool => AssertTool(tool, "bracket", "Bracket Check", "/bracket", ToolNavSection.Analyze, "tool.bracket.enabled", false, "Commander Bracket Checker", "Classify a Commander deck into its official 1–5 bracket using Game Changers, two-card combos, and mass land denial — computed locally, no AI needed.", "bracket", DeckPageTab.Bracket, false),
            tool => AssertTool(tool, "deck-modules", "Deck Modules", "/deck-modules", ToolNavSection.Analyze, "tool.deck-modules.enabled", false, "Deck Modules", "Import one baseline deck, manually assign 2-4 named strategy alternatives with linked mana support, and compile a complete 100-card configuration with an exact swap and reset checklist — no saved projects.", null, DeckPageTab.DeckModules, false, "/deck-modules/import", "/deck-modules/compile", "/deck-modules/export"),
            ...
            tool => AssertTool(tool, "deck-sync", "Deck Sync", "/sync", ToolNavSection.Build, "tool.deck-sync.enabled", false, "Moxfield–Archidekt Deck Sync", "Reconcile a Moxfield deck against an Archidekt deck (either direction) and generate add/cut text for the target.", "deck-sync", DeckPageTab.Sync, false, "/resolve", "/api/deck/diff"),
            tool => AssertTool(tool, "convert", "Convert Deck", "/convert", ToolNavSection.Build, "tool.convert.enabled", false, "MTG Decklist Converter", "Convert deck export text or a public URL between Moxfield and Archidekt formats.", "convert", DeckPageTab.Convert, false),
            ...
            tool => AssertTool(tool, "suggest-categories", "Category Suggestions", "/suggest-categories", ToolNavSection.Categories, "tool.categories.enabled", false, "Commander Deck Tag Suggestions", "Suggest categories for your cards using cached data, Scryfall Tagger, or a reference-deck comparison.", "category-suggestions", DeckPageTab.SuggestCategories, false, "/api/suggestions/card"),
    ...
    [Fact]
    public void All_HasUniqueKeysRoutesAndTabs_AndExactlyThreeCoreTools()
    {
        var registry = new ToolRegistry();

        Assert.Equal(17, registry.All.Count);
        Assert.Equal(17, registry.All.Select(tool => tool.Key).Distinct(StringComparer.Ordinal).Count());
        Assert.Equal(17, registry.All.Select(tool => tool.Route).Distinct(StringComparer.Ordinal).Count());
        Assert.Equal(17, registry.All.Select(tool => tool.Tab).Distinct().Count());
        Assert.Equal(27, registry.All.SelectMany(tool => tool.AdditionalRoutes.Prepend(tool.Route)).Distinct(StringComparer.Ordinal).Count());
        ...
        Assert.Equal(new[] { "deck-analysis", "deck-comparison", "cedh-meta-gap" }, coreKeys);
    }

    [Fact]
    public void AddDeckFlowTools_RegistersSingletonRegistry()
    {
        var services = new ServiceCollection();

        services.AddDeckFlowTools();

        using var provider = services.BuildServiceProvider();
        var first = provider.GetRequiredService<IToolRegistry>();
        var second = provider.GetRequiredService<IToolRegistry>();

        Assert.IsType<ToolRegistry>(first);
        ...
        params string[] additionalRoutes)
    {
        ...
        Assert.Equal(tileTitle, tool.TileTitle);
        Assert.Equal(tileDescription, tool.TileDescription);
        Assert.Equal(helpSlug, tool.HelpSlug);
        Assert.Equal(tab, tool.Tab);
        Assert.Equal(isPrimaryTile, tool.IsPrimaryTile);
        Assert.Equal(key, tool.IconKey);
    }
}
