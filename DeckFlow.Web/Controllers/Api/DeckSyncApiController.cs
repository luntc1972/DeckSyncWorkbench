...
public sealed class DeckSyncApiController : ControllerBase
{
    ...
    public async Task<ActionResult<DeckSyncApiResponse>> PostDiffAsync([FromBody] DeckSyncApiRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!HasMoxfieldInput(request))
        {
            ...
            return BadRequest(new { Message = request.MoxfieldInputSource == DeckFlow.Web.Models.DeckInputSource.PublicUrl ? $"A {leftSystem} deck URL is required." : $"{leftSystem} text is required." });
        }

        if (!HasArchidektInput(request))
        {
            var rightSystem = DeckSyncSupport.GetRightPanelSystem(request.Direction);
            return BadRequest(new { Message = request.ArchidektInputSource == DeckFlow.Web.Models.DeckInputSource.PublicUrl ? $"A {rightSystem} deck URL is required." : $"{rightSystem} text is required." });
        }
    ...
    private static bool HasMoxfieldInput(DeckSyncApiRequest request)
        => request.MoxfieldInputSource == DeckFlow.Web.Models.DeckInputSource.PublicUrl
    ...
    private static bool HasArchidektInput(DeckSyncApiRequest request)
        => request.ArchidektInputSource == DeckFlow.Web.Models.DeckInputSource.PublicUrl
    ...
    {
        if (request.MoxfieldInputSource == DeckFlow.Web.Models.DeckInputSource.PublicUrl
            && !string.IsNullOrWhiteSpace(request.MoxfieldUrl)
