(() => {
    ...
    const move = (from: Panel, to: Panel) => {
        if (!draft || from === to) return; compilation = null; if (draft.analysis) draft.analysisStale = true; const source = panelEntries(from); const destination = panelEntries(to); const picks = selected(from); if (!picks.length) { announce(`Select cards in ${panelTitle(from)} first.`); return; }
    ...
    const addAlternative = () => { if (!draft || draft.alternatives.length >= 4) return; const name = query<HTMLInputElement>('[data-deck-modules-name]')?.value.trim() ?? ''; const profile = query<HTMLSelectElement>('[data-deck-modules-profile]')?.value ?? ''; const playPlan = query<HTMLTextAreaElement>('[data-deck-modules-plan]')?.value.trim() ?? ''; if (!name || !profile || !playPlan) { error('Enter a baseline strategy name, profile, and one-sentence play plan before assigning cards.'); return; } compilation = null; const alternative = { id: crypto.randomUUID?.() ?? `${Date.now()}-${draft.alternatives.length}`, name, profile, playPlan, mainboardEntries: [], manaSupportName: `${name} Mana Support`, manaSupportEntries: [] }; draft.alternatives.push(alternative); draft.selectedAlternativeId = alternative.id; editingAlternative = false; render(); const nameInput = query<HTMLInputElement>('[data-deck-modules-name]'); if (nameInput) { nameInput.value = ''; nameInput.focus(); } const profileInput = query<HTMLSelectElement>('[data-deck-modules-profile]'); if (profileInput) profileInput.value = ''; const planInput = query<HTMLTextAreaElement>('[data-deck-modules-plan]'); if (planInput) planInput.value = ''; };
