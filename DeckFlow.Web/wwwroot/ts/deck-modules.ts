(() => {
    ...
    // WR-16: mirrors the full ConfigurationAnalysisResult JSON contract (server-side in
    // ConfigurationAnalysisResult.cs). `analysisKey?: never` did not express "this member must not
    // exist" -- it declared an optional property of type never, so `{ analysisKey: undefined }`
    // still type-checked; the no-AnalysisKey-on-this-record invariant is already enforced
    // server-side (see the record's remarks and the advisory tests), so it is simply omitted here
    // rather than declared and forbidden.
    type ConfigurationAnalysisSnapshot = { manabaseHandoffKey?: string; configurationId?: string; configurationName?: string; analyzedCardCount?: number; landCount: number; targetLandCount: number; landDelta: number; health: string; rampSourceCount: number; hardToCastCount: number; isCoreOnly: boolean; attributedFindings: { displayColor: string; actualSources: number; requiredSources: number; deficit: number; drivingSpell: string; strength: 'None' | 'ModuleMembership' | 'NamedCard'; attributedCard?: string | null; attributedModule?: string | null; swapDirection?: 'added' | 'removed' | null }[]; unresolvedCardNames?: string[]; analysisNotice?: string | null; signals?: { bracketNumber: number; gameChangers: string[]; massLandDenialCards: string[]; extraTurnCards: string[]; comboDetectionAvailable: boolean; catalogEffectiveDate: string; interactionAttributionAvailable: boolean; interactionsByModule: { moduleName: string; interactionCount: number }[]; declared: { profile: string; playPlan: string; isDeclared: boolean; profileDisagreementNote: string | null } | null } | null };
    ...
    // Why (WR-08): analyze() is the expensive path -- the full manabase pipeline with
    // ClassifyPlanRoles: true, including Scryfall-backed lookups (CLAUDE.md records live
    // Cloudflare IP blocks from impatient double-clicks). These in-flight flags gate
    // Analyze/Compile/Compare so a double-click cannot fire concurrent requests, and late
    // responses cannot land out of order and overwrite a newer result.
    let analyzing = false;
    let compiling = false;
    let comparing = false;
    // WR-11: re-initializing against the same page element previously double-registered every
    // listener and leaked a MutationObserver (observeHandoff() existed only because renderHandoff()
    // was never called from render()/renderAnalysis() directly). Aborting the previous controller before
    // registering a fresh set of listeners makes initialize() idempotent.
    ...
    // Why (WR-15): the server models ManabaseMode as a first-class request field that participates
    // in the cache key, but the UI could only ever post 'Casual' -- a cEDH-declared configuration
    // was analysed against casual thresholds, quietly contradicting the declared-profile disclosure
    // rendered right next to the numbers. Bracket4HighPower maps to Focused (ManabaseMode.cs
    // documents it as the mid-power profile between Casual and Cedh).
    const manabaseModeForProfile = (profile: string | undefined): 'Casual' | 'Focused' | 'Cedh' => profile === 'Cedh' ? 'Cedh' : profile === 'Bracket4HighPower' ? 'Focused' : 'Casual';
    ...
    const renderBalance = () => {
        ...
        const compileButton = query<HTMLButtonElement>('[data-deck-modules-compile]'); if (compileButton) { compileButton.disabled = !okay || compiling; compileButton.setAttribute('aria-describedby', 'deck-modules-balance'); }
        ['export', 'copy', 'analyze'].forEach(action => { const button = query<HTMLButtonElement>(`[data-deck-modules-${action}]`); if (button) { button.disabled = !okay || (action !== 'analyze' && !compilation) || (action === 'analyze' && analyzing); button.setAttribute('aria-describedby', 'deck-modules-balance'); } });
    };
    ...
    const render = () => { const alternative = active(); const summaryProfile = query<HTMLElement>('[data-deck-modules-summary-profile]'); if (summaryProfile) {
        // Why (WR-13): alternative.profile round-trips through sessionStorage and is restored
        // without validation. Building a dynamic attribute selector from it (`option[value="${...}"]`)
        // let a value containing `"` or `]` throw a querySelector SyntaxError, aborting the rest of
        // render() -- panels, balance state, analysis, and the comparison picker all stopped
        // updating with no error surfaced. Look the label up by value comparison instead.
        const profile = query<HTMLSelectElement>('[data-deck-modules-profile]');
        const option = [...(profile?.options ?? [])].find(item => item.value === alternative?.profile);
        summaryProfile.textContent = alternative ? option?.textContent ?? alternative.profile : 'Not selected';
    } const summaryPlan = query<HTMLElement>('[data-deck-modules-summary-plan]'); if (summaryPlan) summaryPlan.textContent = alternative?.playPlan ?? 'Not entered'; if (editingAlternative && alternative) { const name = query<HTMLInputElement>('[data-deck-modules-name]'); if (name) name.value = alternative.name; const profile = query<HTMLSelectElement>('[data-deck-modules-profile]'); if (profile) profile.value = alternative.profile; const plan = query<HTMLTextAreaElement>('[data-deck-modules-plan]'); if (plan) plan.value = alternative.playPlan; } renderCommandZone(); renderAlternatives(); renderPanels(); renderBalance(); renderAnalysis(); renderComparisonPicker(); save(); };
    ...
    // WR-09: the single fetch-options primitive shared by request() and compare()'s local post(),
    // so the header/body construction cannot drift between the two call sites.
    ...
    const request = async (url: string, body: unknown) => { const response = await postJson(url, body); if (!response.ok) { const detail = await response.json().catch(() => ({})) as { message?: string }; throw new Error(detail.message ?? 'Deck Modules request failed.'); } return response; };
    const compile = async () => { if (!draft || !balanced() || compiling) return; compiling = true; compilation = null; renderBalance(); try { const response = await request('/deck-modules/compile', { baselineToken: draft.baselineToken, commandZone: draft.commandZone, baselineMainboardEntries: draft.baselineMainboardEntries, coreEntries: draft.coreEntries, alternatives: draft.alternatives, selectedAlternativeId: draft.selectedAlternativeId }); compilation = await response.json() as Record<string, unknown>; renderReport(compilation); announce('Deck Modules compilation complete.'); } catch (exception) { const message = exception instanceof Error ? exception.message : 'Deck Modules compilation failed.'; error(message.includes('expired') ? `${message} Import again to continue.` : message); } finally { compiling = false; renderBalance(); } };
    ...
    const validateComparisonSelection = () => {
        ...
        if (button) button.disabled = !reference.value || !other.value || same || comparing;
    ...
    const compare = async () => {
        const { reference, other, button, message, head, body } = comparisonElements();
        if (!draft || !reference || !other || !message || !head || !body || comparing) return;
        if (reference.value === other.value) { message.textContent = 'Choose two different alternatives.'; return; }
        // Why (WR-08): disable only the trigger here -- calling the full validateComparisonSelection()
        // (which also clears the message element) in the finally below would stomp whatever
        // message the request just set (e.g. "Analyse Plan B first.").
        comparing = true; if (button) button.disabled = true;
        try {
        ...
        const post = (sides: ComparisonSide[]) => postJson('/deck-modules/compare', { sides, referenceConfigurationId: reference.value });
